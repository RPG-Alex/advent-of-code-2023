...
fn main() {
    //let cards = read_to_string("inputs/day4.txt").unwrap();
    ...
    println!("Solution 1: {:?}", solution_one(cards.clone()));
    println!("Solution 1: {:?}", solution_two(cards));

}

fn solution_two(cards:String) -> u16 {
    let mut total_scratch:u16 = 0;

    let cards = cards.lines();
    total_scratch += cards.clone().count() as u16;
...
fn solution_one(cards: String) -> u16 {

        let mut total_points:u16 = 0;

        cards.lines().for_each(|line| {
        let mut parts = line.split('|');
        let win = parts.next().unwrap_or("failed to unwrap winning numbers");
        let numbers = parts.next().unwrap_or("failed to unwrap your numbers");
        ...
        for num in nums {
            if winning_nums.contains(&num) {
                ...
                current_points *= 2; // Double the points for the next match
